...
readingsSet = set()
readingsList = [item['readings'] for item in fullList]
for list in readingsList:
	readingsSet.update(list)
readingsList = [ {'reading': item} for item in readingsSet]
for item in readingsList:
	if set(item['reading']) <= set(hiragana):
		item['type'] = 'kun'
	elif set(item['reading']) <= set(katakana):
		item['type'] = 'on'
	else:
		item['type'] = 'unknown'
readingsOutput = open('readings.txt', 'w')
readingsOutput.write(json.dumps(readingsList))
readingsOutput.close()